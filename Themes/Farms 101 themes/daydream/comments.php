...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<li class="<?php echo $oddcomment; ?>" id="comment-<?php comment_ID() ?>">
			...
			<?php 
			   ...
			   if (function_exists('gravatar') && get_option('dd_gravatars') == "on") {
				  if ('' != get_comment_author_url()) {
					 ...
					 echo "<a href='http://www.gravatar.com' title='".__('Create your own gravatar at gravatar.com!')."'>";
				  }
				  echo "<img src='";
			...
			<div class="cmntmeta"><?php comment_author_link() ?> - <?php comment_date('F jS, Y') ?> <?php _e('at');?> <?php comment_time() ?></a> <?php edit_comment_link('e','',''); ?></div>
	...
	<?php endforeach; /* end for each comment */ ?>
	 ...
	 <?php else : // this is displayed if there are no comments so far ?>
		...
		<?php if ('open' == $post->comment_status) : ?> 
			...
			<h4&nbsp;&nbsp;&nbsp;<?php _e('There are no comments on this post');?></h4>
	...
	<?php if ('open' == $post->comment_status) : ?>
		...
		<div id="commentformarea" style="text-align: left;">
			...
			<p class="mustbe"><?php _e('You must be');?> <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>"><?php _e('logged in');?></a> <?php _e('to post a comment.');?></p>
			<?php else : ?>
			...
			&nbsp;&nbsp;&nbsp;	<p><input type="text" name="author" id="author" value="<?php echo $comment_author; ?>" size="22" tabindex="1" />
				...
				<label for="email"><small><?php _e('Mail (not published)');?> <?php if ($req) echo "__('(required)')"; ?></small></label></p>
