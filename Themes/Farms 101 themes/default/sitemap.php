...
<?php get_header(); ?>
	...
	<div id="content_box">
		...
		<div id="content" class="pages">
			...
			<h2><?php the_title(); ?></h2>	
			<div class="entry">		
				<p><strong><a href="<?php bloginfo('url'); ?>" alt="<?php bloginfo('name'); ?>"><?php _e('Home');?></a></strong></p>
				<h3><?php _e('All internal pages:');?></h3>
				<ul>
					<?php wp_list_pages('title_li='); ?>
				</ul>
				<h3><?php _e('All internal blog posts'):?>:</h3>
				<ul>
					<?php $archive_query = new WP_Query('showposts=1000');
						while ($archive_query->have_posts()) : $archive_query->the_post(); ?>
					<li><a href="<?php the_permalink() ?>" rel="bookmark" title="<?php _e('Permanent Link to');?> <?php the_title(); ?>"><?php the_title(); ?></a> <strong><?php comments_number('0', '1', '%'); ?></strong></li>
					<?php endwhile; ?>
				</ul>
				<h3><?php _e('Monthly archive pages:');?></h3>
				<ul>
					<?php wp_get_archives('type=monthly'); ?>
				</ul>
				<h3><?php _e('Topical archive pages:');?></h3>
				<ul>
					<?php wp_list_categories('title_li=0'); ?>
				</ul>
				<h3><?php _e('Available RSS Feeds'):?>:</h3>
				<ul>
