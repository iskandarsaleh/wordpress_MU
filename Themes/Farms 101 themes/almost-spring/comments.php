...
<?php if ($comments) : ?>
	...
	<?php foreach ($comments as $comment) : ?>
		...
		<h3 class="commenttitle"><?php if (function_exists('avatar_display_comments')){ avatar_display_comments(get_comment_author_email(),'48',''); } ?> <?php comment_author_link() ?> <?php _e('Said'); ?>,</h3>
...
<?php if ('open' == $post-> comment_status) : ?>
	...
	<h2 id="postcomment"><?php _e('Leave a Comment'); ?></h2><?php if (function_exists('comment_form_text_output')){ comment_form_text_output(); } ?>
	...
	<?php else : ?>
		...
		<label for="email"><?php _e('E-mail');?> (<?php if ($req) _e('required, '); ?> <?php _e('never displayed');?>)</label>
		</p>
		
		<p>
		<input type="text" name="url" id="url" value="<?php echo $comment_author_url; ?>" size="30" tabindex="3" />
		<label for="url"><abbr title="<?php _e('Uniform Resource Identifier');?>">URI</abbr></label>
		</p>
