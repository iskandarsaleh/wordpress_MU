...
function BX_archive()
{
	...
	if ($arcresults) {
		foreach ($arcresults as $arcresult) {
        	...
        	$arcresults2 = $wpdb->get_results("SELECT ID, post_date, post_title, comment_status FROM " . $wpdb->posts . " WHERE post_date LIKE '%-$thismonth-%' AND post_status='publish' ORDER BY post_date DESC");
        	...
        	if ($arcresults2) {
            	...
            	foreach ($arcresults2 as $arcresult2) {
               		if ($arcresult2->post_date != '0000-00-00 00:00:00') {
						...
						$coments_count = mysql_num_rows($comments);
						if ($arcresult2->comment_status == "open" OR $coments_count > 0) echo '&nbsp;('.$coments_count.')';
						echo "</li>\n";
                 	}
            	}
            	echo "</ul>\n";
