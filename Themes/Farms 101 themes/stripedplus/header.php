...
<div id="bigwrapper">
	...
	<div id="topbar" onmouseover="scale.sizeup()" onmouseout="scale.sizedown(30)"><script type="text/javascript">var topbar = document.getElementById('topbar'); topbar.style.height = 30 + "px";</script>
		<div id="redline"><p>Navigation | <span class="where"><?php if ( is_search() ) { ?> Search results &raquo; <?php echo wp_specialchars($s, 1); ?><?php } ?><?php if ( is_date() ) { ?><?php _e('Archive');?><?php wp_title(); ?><?php } ?><?php if ( is_category() ) { ?> Category <?php wp_title(); ?><?php } ?><?php if ( is_single() or is_page()) { ?><?php wp_title(''); ?><?php } ?><?php if ( is_home()) { ?><?php bloginfo('title'); ?><?php } ?></span></p></div>
