<?php if (function_exists('comment_form_text_output')){ comment_form_text_output(); } ?><br /><div id="comments">
...
<?php foreach ($comments as $comment) : ?>
				...
				<li id="comment-<?php comment_ID() ?>" class="<?php echo $oddcomment; ?>">
					<?php if (function_exists('avatar_display_comments')){ avatar_display_comments(get_comment_author_email(),'48',''); } ?>&nbsp;&nbsp;<strong><?php comment_author_link() ?></strong> wrote:
					<?php if ($comment->comment_approved == '0') : ?><em><?php _e('Your comment is awaiting moderation.');?></em><?php endif; ?>
					<?php comment_text() ?>
					<small class="commentmetadata"><?php comment_date('F jS, Y,') ?> <?php _e('at');?> <?php comment_time() ?> <a href="#comment-<?php comment_ID() ?>" title="Permalnik to this comment" rel="permalink">#</a> <?php edit_comment_link('e','',''); ?></small>
				</li>
	...
	<div id="col1" class="sidebar">
		<ul>
			<li>
				...
				<p id="single-post-metadata">This entry was <?php _e('Posted by');?> <?php the_author(); ?> <?php _e('on');?> <?php the_time('l, F jS, Y') ?>, at <?php the_time() ?>, and was filed in <?php the_category(', ') ?>.</p>
...
<?php if ( get_option('comment_registration') && !$user_ID ) : ?>
		...
		<p><?php _e('You must be');?> <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?redirect_to=<?php the_permalink(); ?>" title="Log in">logged in</a> to post a comment.</p>
...
<?php if ( $user_ID ) : ?>
			...
			<p><?php _e('Logged in as');?> <a href="<?php echo get_option('siteurl'); ?>/wp-admin/profile.php" title="Logged in as <?php echo $user_identity; ?>"><?php echo $user_identity; ?></a>. <a href="<?php echo get_option('siteurl'); ?>/wp-login.php?action=logout" title="<?php _e('Log out of this account');?>"><?php _e('Logout');?> &raquo;</a></p>
