...
<?php while (have_posts()) : the_post(); ?>
		...
		<div id="post-<?php the_ID(); ?>" class="post">
			...
			<div class="post-footer">
				<p class="post-metadata">Filed in <?php the_category(', ') ?> | <a href="<?php the_permalink() ?>" title="Permalink to <?php the_title(); ?>" rel="permalink">Permalink</a> |<?php the_tags( '&nbsp;' . __( 'Tagged' ) . ' ', ', ', ''); ?> <?php edit_post_link(__('Edit'), '', ' | '); ?>  <?php comments_popup_link('Comments (0)', 'Comments (1)', 'Comments (%)'); ?></p>
			</div><!-- END POST-FOOTER -->
		</div><!-- END POST -->
		<!-- <?php trackback_rdf(); ?> -->
