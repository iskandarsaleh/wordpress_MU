<?php get_header(); ?>
	...
	<div id="content_box">
		...
		<?php if (have_posts()) : while (have_posts()) : the_post(); ?>
			...
			<h1><?php the_title(); ?></h1>
			<p class="post_date"><?php the_time('F jS, Y') ?> | <?php the_category(', ') ?> | <?php the_tags( '&nbsp;' . __( 'Tagged' ) . ' ', ', ', ''); ?></p>
			<div class="entry"><?php if (function_exists('wp_ozh_wsa')) { wp_ozh_wsa("336280nocolor"); } ?>
				<?php the_content('<p>Read the rest of this entry &raquo;</p>'); ?><?php if (function_exists('wp_ozh_wsa')) { wp_ozh_wsa("336280nocolor"); } ?>
