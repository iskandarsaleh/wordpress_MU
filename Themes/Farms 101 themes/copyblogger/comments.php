...
<div id="comments">
	...
	<?php $count_pings = 1; foreach ($comments as $comment) { ?>
		...
		<dt id="comment-<?php comment_ID() ?>"<?php if ($comment->comment_author_email == get_the_author_email()) { ?> class="author"<?php } ?>>
			...
			<?php if (function_exists('avatar_display_comments')){ avatar_display_comments(get_comment_author_email(),'48',''); } ?>&nbsp;&nbsp;<strong><?php comment_author_link() ?> </strong><?php _e('on');?> <?php comment_date('m.d.y') ?> <?php _e('at');?> <?php comment_time() ?>
		</dt>
	...
	<?php if ('open' == $post-> comment_status) : ?>
		...
		<?php else : ?>
			...
			<?php } ?>
				...
				<p>
					<input name="submit" class="form_submit" type="submit" id="submit" tabindex="5" value="<?php _e('Submit');?>" />
