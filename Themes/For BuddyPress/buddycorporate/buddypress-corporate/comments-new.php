...
// Do not delete these lines
	...
	if ( post_password_required() ) { ?>
		<h2 id="post-header"><?php _e('This post is password protected. Enter the password to view comments.'); ?></h2>
	<?php
		return;
	}
?>
...
<h4 id="comments"><span><?php comments_number(__('No Comments Yet'), __('1 Comment Already'), __('% Comments Already')); ?></span></h4>
...
<h4><span><?php _e('Trackbacks/Pingbacks'); ?></span></h4>
...
<h2 id="post-header"><?php _e('Sorry, the comment form is closed at this time.'); ?></h2>
...
<h4><span><?php comment_form_title(__('Leave a Reply'), __('Leave a Reply to %s') ); ?></span></h4>
...
<p>
<label for="author"><small><?php _e('Name'); ?> <?php if ($req) _e('(required)'); ?></small></label><br />
<input type="text" class="tf" name="author" id="author" value="<?php echo $comment_author; ?>" <?php if ($req) echo "aria-required='true'"; ?> />
</p>

<p>
<label for="email"><small><?php _e('Mail (will not be published)');?> <?php if ($req) _e('(required)'); ?></small></label><br />
...
<p>
<input name="submit" type="submit" class="st" value="<?php echo attribute_escape(__('Submit Comment')); ?>" id="submit" alt="submit" />
