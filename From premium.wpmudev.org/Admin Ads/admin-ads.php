...
function admin_ads_plug_pages() {
	...
	if ( is_site_admin() ) {
		add_submenu_page('wpmu-admin.php', 'Admin Ads', 'Admin Ads', 10, 'admin-ads', 'admin_ads_page_main_output');
	}
}
...
function admin_ads_page_main_output() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
            ...
            <form method="post" action="wpmu-admin.php?page=admin-ads&action=update">
		...
		case "update":
			if ( isset( $_POST[ 'Reset' ] ) ) {
				...
				window.location='wpmu-admin.php?page=admin-ads&updated=true&updatedmsg=" . urlencode(__('Settings cleared.')) . "';
				</script>
				";
			} else {
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='wpmu-admin.php?page=admin-ads&updated=true&updatedmsg=" . urlencode(__('Settings saved.')) . "';
				</script>
				";
			}
		break;
