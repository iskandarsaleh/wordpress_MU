...
function content_monitor_plug_pages() {
	global $wpdb, $wp_roles, $current_user;
	if ( is_site_admin() ) {
		add_submenu_page('wpmu-admin.php', 'Content Monitor', 'Content Monitor', 10, 'content-monitor', 'content_monitor_page_main_output');
	}
}
...
function content_monitor_page_main_output() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
            ...
            <form method="post" action="wpmu-admin.php?page=content-monitor&action=update">
		...
		case "update":
			...
			window.location='wpmu-admin.php?page=content-monitor&updated=true&updatedmsg=" . urlencode(__('Settings saved.')) . "';
			</script>
			";
		break;
		//---------------------------------------------------//
