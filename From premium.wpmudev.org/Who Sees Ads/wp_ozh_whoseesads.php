...
function wp_ozh_wsa_addmenu() {
	...
	require_once(ABSPATH.'wp-content/mu-plugins/'.dirname($wp_ozh_wsa['path']).'/wp_ozh_whoseesads_admin.php');

	// Add submenu page where applicable 
	if (isset($wp_ozh_wsa['my_menu'])) {
		if ( $wp_ozh_wsa['my_menu'] == 'wpmu-admin.php' ) {
			if ( is_site_admin() ) {
...
function wp_ozh_wsa_saveoptions() {
	global $wp_ozh_wsa, $current_site, $wpdb;
	
	if ( is_site_admin() ) {	
		$site_main_blog_id = $wpdb->get_var( "SELECT blog_id FROM {$wpdb->blogs} WHERE domain = '" . $current_site->domain . "' AND path = '" . $current_site->path . "'" );
		switch_to_blog($site_main_blog_id);
...
function wp_ozh_wsa_plugin_basename($file) {
	...
	$file = preg_replace('|^.*/wp-content/mu-plugins/|','',$file); // get relative path from plugin dir
	return $file;
}
