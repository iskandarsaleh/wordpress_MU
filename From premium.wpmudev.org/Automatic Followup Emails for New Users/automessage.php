...
class automessageadmin {
	...
	function setup_listeners() {
		...
		if ( !wp_next_scheduled('process_automessage_hook')) {
			wp_schedule_event(time(), 'daily', 'process_automessage_hook');
		}
	...
	function edit_action_form($id) {
		...
		echo '<th style="" scope="row">' . __('Action','automessage') . '</th>';
		echo '<td>';
		...
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row">' . __('Message delay','automessage') . '</th>';
		echo '<td>';
		...
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row">' . __('Message Subject','automessage') . '</th>';
		echo '<td><input name="subject" type="text" size="50" title="' . __('Message subject') . '" style="width: 50%;" value="' . htmlentities(stripslashes($editing->subject),ENT_QUOTES, 'UTF-8') . '" /></td>';
		echo '</tr>';
			
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row">' . __('Message','automessage') . '</th>';
		echo '<td><textarea name="message" style="width: 50%; float: left;" rows="15" cols="40">' . htmlentities(stripslashes($editing->message),ENT_QUOTES, 'UTF-8') . '</textarea>';
	...
	function add_action_form() {
		...
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row">' . __('Message Subject','automessage') . '</th>';
		echo '<td><input name="subject" type="text" size="50" title="' . __('Message subject') . '" style="width: 50%;" /></td>';
		echo '</tr>';
			
		echo '<tr class="form-field form-required">';
		echo '<th style="" scope="row">' . __('Message','automessage') . '</th>';
		echo '<td><textarea name="message" style="width: 50%; float: left;" rows="15" cols="40"></textarea>';
		// Display some instructions for the message.
		echo '<div class="instructions" style="float: left; width: 40%; margin-left: 10px;">';
		echo __('You can use the following constants within the message body to embed database information.','automessage');
		echo '<br /><br />';
	...
	function append_global_cronjobs($cron) {
		...
		$globalcron = get_site_option('global_cron', false);
				...
				if(is_array($cron)) {
					foreach($cron as $k => $value) {
						if($k == 'process_automessage_hook') {
							...
							update_site_option( 'global_cron', $global);
	...
	function process_schedule() {
		...
		AND s.pause = 0 AND runon <= $tstamp
		ORDER BY runon" );
	...
	function force_process($schedule_id) {
		...
		$sql = $this->db->prepare( "SELECT q.*, s.subject, s.message, s.period, s.timeperiod, s.action_id  FROM {$this->am_queue} AS q, {$this->am_schedule} AS s, {$this->am_actions} AS a 
		WHERE q.schedule_id = s.id AND a.id = s.action_id
		AND s.pause = 0 AND q.schedule_id <= %d
		ORDER BY runon", $schedule_id );
