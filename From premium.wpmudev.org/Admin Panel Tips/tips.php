...
function tips_plug_pages() {
	if ( is_site_admin() ){
		add_submenu_page('wpmu-admin.php', 'Tips', 'Tips', 10, 'manage-tips', 'tips_manage_output' );
	}
}
...
function tips_manage_output() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		$tmp_tips_count = $wpdb->get_var("SELECT COUNT(*) FROM " . $wpdb->base_prefix . "tips WHERE tip_site_ID = '" . $current_site->id . "'");
            ...
            <h2><?php _e('Manage Tips') ?> (<a href="wpmu-admin.php?page=manage-tips&action=new_tip"><?php _e('New') ?></a>):</h2>
            <?php
			if ($tmp_tips_count == 0){
			?>
            <p><?php _e('Click ') ?><a href="wpmu-admin.php?page=manage-tips&action=new_tip"><?php _e('here') ?></a><?php _e(' to add a new tip.') ?></p>
            <?php
			} else {
			...
			if (count($tmp_tips) > 0){
				...
				echo "<td valign='top'><a href='wpmu-admin.php?page=manage-tips&action=edit_tip&tid=" . $tmp_tip['tip_ID'] . "' rel='permalink' class='edit'>" . __('Edit') . "</a></td>";
				echo "<td valign='top'><a href='wpmu-admin.php?page=manage-tips&action=delete_tip&tid=" . $tmp_tip['tip_ID'] . "' rel='permalink' class='delete'>" . __('Remove') . "</a></td>";
				echo "</tr>";
				$class = ('alternate' == $class) ? '' : 'alternate';
				//=========================================================//
				}
		...
		?>
            ...
            <form name="form1" method="POST" action="wpmu-admin.php?page=manage-tips&action=new_tip_process"> 
                <table class="form-table">
                <tr valign="top">
		...
		case "new_tip_process":
			if ($_POST['tip_content'] == ''){
				...
				window.location='wpmu-admin.php?page=manage-tips&updated=true&updatedmsg=" . urlencode(__('Tip Added!')) . "';
				</script>
				";
			}
		break;
		...
		case "edit_tip":
            ...
            <form name="form1" method="POST" action="wpmu-admin.php?page=manage-tips&action=edit_tip_process"> 
		...
		case "edit_tip_process":
			...
			if ($tmp_tip_count > 0){
				if ($_POST['tip_content'] == ''){
					...
					echo "
					<SCRIPT LANGUAGE='JavaScript'>
					window.location='wpmu-admin.php?page=manage-tips&updated=true&updatedmsg=" . urlencode(__('Settings saved.')) . "';
					</script>
					";
				}
			}
		...
		case "delete_tip":
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='wpmu-admin.php?page=manage-tips&updated=true&updatedmsg=" . urlencode(__('Tip Removed.')) . "';
				</script>
				";
		break;
		//---------------------------------------------------//
