...
<channel>
	...
	if ( count( $posts ) > 0 ) {
		foreach ($posts as $post) {
			$author_user_login = $wpdb->get_var("SELECT user_login FROM " . $wpdb->base_prefix . "users WHERE ID = '" . $post['post_author'] . "'");
			?>
			<item>
				...
				<dc:creator><?php echo $author_user_login; ?></dc:creator>
