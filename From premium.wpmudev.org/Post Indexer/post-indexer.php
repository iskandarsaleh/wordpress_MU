...
$post_indexer_current_version = '1.0.4';
//------------------------------------------------------------------------//
//---Config---------------------------------------------------------------//
//------------------------------------------------------------------------//
...
function post_indexer_post_terms_insert_update($tmp_sort_terms,$tmp_post_terms){
		...
		foreach ( $tmp_post_terms as $tmp_post_term ) {
			if ( $tmp_sort_term != '' && $tmp_post_term != '' ) {
				...
				if ( $tmp_exists != 1 ) {
					...
					VALUES
					('" . $tmp_post_term . "','" . $tmp_sort_term . "','" . time() . "','0')");
				}
			}
		}
	}
    ...
    $update_query = "UPDATE " . $wpdb->base_prefix . "term_counts SET term_count = term_count + 1,term_count_updated = '" . time() . "' ". $update_where . " )";
