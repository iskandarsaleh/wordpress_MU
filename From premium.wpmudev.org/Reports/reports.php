...
function reports_plug_pages() {
	if ( is_site_admin() ) {
		add_submenu_page('wpmu-admin.php', __('Reports'), __('Reports'), 10, 'reports', 'reports_page_output');
	}
}
...
function reports_page_output() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
			...
			if ( count( $available_reports ) > 0 ) {
				...
				if ( count( $available_reports ) > 0 ) {
					...
					echo "<td valign='top'><a href='wpmu-admin.php?page=reports&action=view-report&report=" . $available_report[1] . "' rel='permalink' class='edit'>" . __('View Report') . "</a></td>";
		...
		case "view-report":
			...
			?>
			<h2><a href="wpmu-admin.php?page=reports" style="text-decoration:none;"><?php _e('Reports') ?></a> &raquo; <a href="wpmu-admin.php?page=reports&action=view-report&report=<?php echo $report_nicename; ?>" style="text-decoration:none;"><?php _e($report_name); ?></a></h2>
            <?php
