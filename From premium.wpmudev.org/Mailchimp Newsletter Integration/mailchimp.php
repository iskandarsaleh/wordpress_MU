...
function mailchimp_plug_pages() {
	...
	if ( function_exists("is_site_admin") ) {
		if ( is_site_admin() ) {
			add_submenu_page('wpmu-admin.php', 'MailChimp', 'MailChimp', 10, 'mailchimp', 'mailchimp_settings_page_output');
		}
	} else {
...
function mailchimp_settings_page_output() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		default:
			...
			if ( function_exists("is_site_admin") ) {
			?>
            <form method="post" action="wpmu-admin.php?page=mailchimp&action=process">
            <?php
			} else {
			?>
            ...
            if ( count( $mailchimp_lists ) > 0 || is_array( $mailchimp_lists ) ) {
				...
				if ( function_exists("is_site_admin") ) {
				?>
				<form method="post" action="wpmu-admin.php?page=mailchimp&action=import-process">
				<?php
				} else {
				?>
		...
		case "process":
			...
			if ( function_exists("is_site_admin") ) {
				...
				window.location='wpmu-admin.php?page=mailchimp&updated=true&updatedmsg=" . urlencode(__('Settings saved.')) . "';
				</script>
				";
			} else {
				echo "
		...
		case "import-process":
			...
			if ( function_exists("is_site_admin") ) {
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='wpmu-admin.php?page=mailchimp&updated=true&updatedmsg=" . urlencode(__('Existing users imported.')) . "';
				</script>
				";
			} else {
