...
function avatars_site_admin_column_content($column,$user_ID) {
	...
	if ( $column == 'avatar' ) {
		...
		?>
        <br />
        (<a href="wpmu-admin.php?page=edit-user-avatar&uid=<?php echo $user_ID; ?>" style="text-decoration:none;"><?php _e('Edit'); ?></a>)
        <?php
	}
}
...
function avatars_plug_pages() {
	...
	if ( is_site_admin() && $_GET['page'] == 'edit-user-avatar' ) {
		add_submenu_page('wpmu-admin.php', 'Edit User Avatar', __('Edit User Avatar'), 'read', 'edit-user-avatar', 'avatars_page_site_admin_edit_user_avatar' );
	}
}
...
function avatars_page_site_admin_edit_user_avatar() {
	...
	switch( $_GET[ 'action' ] ) {
		...
		?>
			...
			<form action="wpmu-admin.php?page=edit-user-avatar&action=upload_process&uid=<?php echo $_GET['uid']; ?>" method="post" enctype="multipart/form-data">
			<p>
            <?php
		...
		case "upload_process":
			if ( isset( $_POST['Reset'] ) ) {
				...
				echo "
				<SCRIPT LANGUAGE='JavaScript'>
				window.location='wpmu-admin.php?page=edit-user-avatar&uid=" . $_GET['uid'] . "&updated=true&updatedmsg=" . urlencode(__('Avatar reset.')) . "';
				</script>
				";
			} else {
				$avatar_path = ABSPATH . $user_avatars_path . substr(md5($_GET['uid']), 0, 3) . '/';
				...
				if ( isset( $_POST['Alternative'] ) ) {
					...
					echo "
					<SCRIPT LANGUAGE='JavaScript'>
					window.location='wpmu-admin.php?page=edit-user-avatar&uid=" . $_GET['uid'] . "&updated=true&updatedmsg=" . urlencode(__('Avatar updated.')) . "';
					</script>
					";
				} else {
					//Standard Upload
					?>
					<h2><?php _e('Crop Image') ?></h2>
					<form method="post" action="wpmu-admin.php?page=edit-user-avatar&uid=<?php echo $_GET['uid']; ?>&action=crop_process">
